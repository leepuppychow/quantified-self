...
class FoodsHandler extends Handler {
  ...
  renderAllFoods(option=0) {
    var sorted = [...this.foods.values()].sort(Food.sortOptions[option])
    sorted.forEach(this.prependFood)
  }

  storeFoods(foods) {
    foods.forEach((food) => {
      this.addFoodToMap(food)
    })
  }
  ...
  handleSubmitAddFood(event) {
    ...
    if (!errorText.length) {
      this.service.create(food)
        .then((food) => {
          this.addFoodToMap(food)
          this.prependFood(food)
        })
      inputs.name.val('')
      inputs.calories.val('')
    }
